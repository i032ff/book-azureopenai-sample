...
import openai  # 本家のOpenAIクライアントをインポート
import tiktoken
...
def upload_blobs(filename):
    ...
    if os.path.splitext(filename)[1].lower() == ".pdf":
        ...
        for i in range(len(pages)):
            ...
            if args.verbose:
                logger.info(f"\tUploading blob for page {i} -> {blob_name}")
            f = io.BytesIO()
            writer = PdfWriter()
...
def remove_blobs(filename):
    if args.verbose:
        logger.info(f"Removing blobs for '{filename or '<all>'}'")
    blob_service = BlobServiceClient(
        account_url=f"https://{args.storageaccount}.blob.core.windows.net", credential=storage_creds)
    blob_container = blob_service.get_container_client(args.container)
...
def compute_embedding(text):
    response = openai_client.Embedding.create(
        input=text, model=args.openaimodelname)
    return response['data'][0]['embedding']


@retry(wait=wait_random_exponential(min=15, max=60), stop=stop_after_attempt(15), before_sleep=before_retry_sleep)
def compute_embedding_in_batch(texts):
    emb_response = openai_client.Embedding.create(
        input=texts, model=args.openaimodelname)
    return [data['embedding'] for data in emb_response['data']]


def create_search_index():
    if args.verbose:
        logger.info(f"Ensuring search index {args.index} exists")
    ...
    if args.index not in index_client.list_index_names():
        ...
        if args.verbose:
            logger.info(f"Creating {args.index} search index")
        index_client.create_index(index)
    else:
        if args.verbose:
            logger.info(f"Search index {args.index} already exists")
    ...
    for filename in glob.glob(path_pattern):
        ...
        if args.remove:
            ...
            try:
                ...
                sections = create_sections(os.path.basename(
                    filename), page_map, use_vectors and not vectors_batch_support, category)
                ...
                logger.info(
                    f"\tGot an error while reading {filename} -> {e} --> skipping file")
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--searchservice", help="Name of the Azure AI Search service where content should be indexed (must exist already)")
    parser.add_argument(
        "--index", help="Name of the Azure AI Search index where content should be indexed (will be created if it doesn't exist)")
    ...
    parser.add_argument("--openaikey", required=True,
                        help="OpenAI API key for accessing the OpenAI service")
    parser.add_argument("--novectors", action="store_true",
                        help="Don't compute embeddings for the sections (e.g. don't call the OpenAI embeddings API during indexing)")
    parser.add_argument("--disablebatchvectors", action="store_true",
    ...
    parser.add_argument("--localpdfparser", action="store_true",
                        help="Use PyPdf local PDF parser (supports only digital PDFs) instead of Azure AI Document Intelligence service to extract text, tables and layout from the documents")
    ...
    parser.add_argument("--formrecognizerkey", required=False,
                        help="Optional. Use this Azure AI Document Intelligence account key instead of the current user identity to login (use az login to set current user for Azure)")
    ...
    azd_credential = AzureDeveloperCliCredential() if args.tenantid is None else AzureDeveloperCliCredential(
    ...
    search_creds = default_creds if args.searchkey is None else AzureKeyCredential(
        args.searchkey)
    ...
    if not args.localpdfparser:
        ...
        if args.formrecognizerservice is None:
            logger.info(
                "Error: Azure AI Document Intelligence service is not provided. Please provide formrecognizerservice or use --localpdfparser for local pypdf parser.")
            exit(1)
        formrecognizer_creds = default_creds if args.formrecognizerkey is None else AzureKeyCredential(
